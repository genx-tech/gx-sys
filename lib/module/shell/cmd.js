...
export const run_ = (cmd, options) => new Promise((resolve, reject) => childProcess.exec(cmd, {
  windowsHide: true,
  ...options
}, (error, stdout, stderr) => {
  if (error) {
...
export const runLive_ = (cmd, args, onStdOut, onStdErr, options) => new Promise((resolve, reject) => {
  ...
  if (onStdOut) {
    ...
    (_onStdErr = onStdErr) !== null && _onStdErr !== void 0 ? _onStdErr : onStdErr = onStdOut;
  } else {
...
export const runSync = (cmd, options) => childProcess.execSync(cmd, {
  windowsHide: true,
  ...options
}).toString();
